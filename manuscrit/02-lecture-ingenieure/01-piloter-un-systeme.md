...
[Sommaire](../SOMMAIRE.md) · [Lire la version accessible](../01-lecture-accessible/01-piloter-un-systeme.md) · [Fiche de rédaction](../../tranches/B01-piloter-un-systeme.md)
...
Face à cette réalité, la responsabilité de l'ingénieur ne consiste plus à rivaliser de vitesse de frappe au clavier. Elle consiste à concevoir le **[Harnais](../03-annexes/05-glossaire.md#harnais) cybernétique** qui entoure l'agent.

Dans la théorie du contrôle en boucle fermée[^2], un système automatisé ne peut rester stable que si son régulateur possède au moins autant de variété que les perturbations qu'il doit compenser (Loi de variété requise de W. Ross Ashby). L'[Agent](../03-annexes/05-glossaire.md#agent) est un actionneur stochastique ultra-rapide. Le harnais — composé des types stricts, des linters syntaxiques, des bacs à sable d'exécution et des suites de tests unitaires — constitue le régulateur déterministe. 
...
Un système robuste formalise au contraire une **machine à états finis** (*Finite State Machine*) où chaque transition est adossée à une autorité, une précondition, un [Invariant](../03-annexes/05-glossaire.md#invariant) et une trace observable :
...
- **I-MD §1.1 à §1.5** ([Manuel d'Ingénierie Logicielle](../../sources/originaux/MANUEL_INGENIERIE_LOGICIELLE_2026_COMPLET.md)) : Thermodynamique du code IA, dissipation d'attention (*The Haystack Hazard*), analyse statique (AST) et protocoles d'isolation d'exécution.
- **I-MD §9.5** ([Manuel d'Ingénierie Logicielle](../../sources/originaux/MANUEL_INGENIERIE_LOGICIELLE_2026_COMPLET.md)) : Les quatre compétences souveraines de l'orchestrateur (pensée modulaire, rigueur schema-first, acuité visuelle du diff, refus de l'approximation).
- **O-MD §1, §4 et §5** ([Manuel d'Orchestration Logicielle](../../sources/originaux/manuel_orchestration_logicielle.md)) : Organisation systémique de l'atelier logiciel, protocole de dialogue en sept messages et l'ordre juste pour construire sous preuve.

Pour explorer l'organisation des frontières logiques et la conception des modules profonds, poursuis vers le chapitre suivant : **[B02 — Organiser l'architecture et les responsabilités](../02-lecture-ingenieure/02-architecture-et-frontieres.md)**. Pour revoir les concepts sans prérequis de code, consulte le miroir accessible : **[A01 — Piloter un système, pas une génération de code](../01-lecture-accessible/01-piloter-un-systeme.md)**.
