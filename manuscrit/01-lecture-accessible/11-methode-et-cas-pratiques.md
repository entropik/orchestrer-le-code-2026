...
[Sommaire](../SOMMAIRE.md) · [Approfondir le même chapitre](../02-lecture-ingenieure/11-methode-et-cas-pratiques.md) · [Fiche de rédaction](../../tranches/A11-methode-et-cas-pratiques.md)
...
Tant que le travail reste dans le périmètre autorisé, l'agent avance sous surveillance. Dès qu'une frontière critique est approchée, tu tranches par un court arbitrage écrit ([ADR](../03-annexes/05-glossaire.md)).
...
3. **Sécuriser à la source** : Le serveur impose une [clé d'idempotence](../03-annexes/05-glossaire.md) associée au panier et une règle d'unicité absolue dans la base de données.
...
- **O-MD §5, §11, §12, §13 et §14** ([Manuel d'Orchestration Logicielle](../../sources/originaux/manuel_orchestration_logicielle.md)) : La méthode ORCHESTRE en 9 points, le point d'arrêt décisionnel, les cas pratiques de création de fonctionnalité et de diagnostic de bug.
- **I-MD §9 et §10** ([Manuel d'Ingénierie Logicielle](../../sources/originaux/MANUEL_INGENIERIE_LOGICIELLE_2026_COMPLET.md)) : Le minimalisme radical (KISS), les six phases séquentielles de livraison, la bibliothèque de prompts déterministes et la grille d'audit de PR.

Pour découvrir l'implémentation technique des tranches verticales avec exécution concurrente multithreadée sous barrière, simulation d'upload multipart et gestion des courses critiques en Python 3.11, poursuis vers le chapitre miroir : **[B11 — Appliquer ORCHESTRE du besoin à la résolution](../02-lecture-ingenieure/11-methode-et-cas-pratiques.md)**.
